...
---
  ...
  # Load libraries and print session info to facilitate reproducibility.  This was run on DMAP memory intensive 16xLarge (64 cpus, 512GB) instance.  
...
# rename climate field
# fl.sf <- fl.sf %>% dplyr::rename(climate = fldd_zn)
```

### Areas and State Names
All area calculations were performed in Goode Homolosine Land "ESRI:54052" projection.  All layers were projected to WGS84 after the area calculations.
...
# Text for report
###Flooded Land Remaining Flooded Land
#### Reservoirs: Emissions and Removals from Flooded Land Remaining Flooded Land
In 2022 the conterminous United States hosted `r fl.cl.area.state.type %>% filter(lu == "Flooded Land Remaining Flooded Land", type == "reservoir") %>% select(area2022) %>% summarize(tot.area = sum(.)) %>% pull() / 1000000` million hectares of reservoirs in the Flooded Land Remaining Flooded Land category.  These reservoirs are distributed across all `r fl.cl.area.state.type %>% filter(lu == "Flooded Land Remaining Flooded Land", type == "reservoir")  %>% summarize(climate.zones = length(unique(climate))) %>% pull()` six of the aggregated climate zones used to define flooded land emission factors.

See "#####TABLE:: FLRFL-reservoirs CH4 emissions by mechanism (dn, surface) and state" below for stats on relative emissions from Texas, Florida, and Louisiana.

Methane emissions from reservoirs in Flooded Land Remaining Flooded Land increased `r flrfl.ch4.national.subtype %>% clean_names %>% filter(subtype == "reservoir", flux_type == "total") %>% mutate(percent.increase = ((x2022 - x1990)/x1990)*100) %>% select(percent.increase) %>% pull()` percent from 1990 to 2022 due to the matriculation of reservoirs in Land Converted to Flooded Land to Flooded Land Remaining Flooded Land.
...
flrfl.ch4.national.subtype %>% 
  ...
  mutate(flux.type = factor(flux.type, c("surface", "downstream", "total")), # for preferred row arrangemen
         across(where(is.numeric), round)) %>%
  arrange(flux.type) %>%
  # Create GT table of data
  gt() %>%
  tab_header(title = md(
    "CH4 Emissions from Flooded Land Remaining Flooded Land—Reservoirs"), 
...
#### Other Constructed Waterbodies: Emissions and Removals from Flooded Land Remaining Flooded Land
...
#####TABLE
Comparison of emission estimates in 1990-2021 vs 1990-2022 Inventory.  This table does not appear in any spreadsheet, only in text.  Manually type results into Word table. 
...
##### Area
For the year 2022, this inventory contains `r fl.cl.area.state.type  %>%  filter(lu == "Land Converted to Flooded Land", subtype == "freshwater pond") %>% summarize(area2022 = sum(area2022, na.rm = TRUE)) %>% pull()` ha of freshwater ponds and `r fl.cl.area.state.type  %>%  filter(lu == "Land Converted to Flooded Land", subtype == "canals and ditches") %>% summarize(area2022 = sum(area2022, na.rm = TRUE)) %>% pull()` ha of canals and ditches in Flooded Land Remaining Flooded Land.
Area of freshwater ponds decreased by `r (fl.cl.area.state.type  %>%  filter(lu == "Land Converted to Flooded Land", subtype == "freshwater pond") %>% summarize(area2022 = sum(area2022, na.rm = TRUE)) %>% pull() - fl.cl.area.state.type  %>%  filter(lu == "Land Converted to Flooded Land", subtype == "freshwater pond") %>% summarize(area1990 = sum(area1990)) %>% pull()) / fl.cl.area.state.type  %>%  filter(lu == "Land Converted to Flooded Land", subtype == "freshwater pond") %>% summarize(area1990 = sum(area1990)) %>% pull()` 95 percent Ha as lands matriculated out of the LCFL category.
