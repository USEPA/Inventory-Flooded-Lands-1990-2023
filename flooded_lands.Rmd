...
#fl.sf <- st_read("output/2023_Flooded_Lands_Inventory_Final_121923.gpkg")
```
...
Total CH4 emissions from reservoirs in FLRFL are shown in the text in both kt CH4 and MMT CH4-CO2-eq.  These tables are the top two tables in the "tables" tab of the spreadsheet and are calculated in Excel.  Lets create the tables here. Double check against Excel to confirm calculations.
...
gt(flrfl.ch4.2022.reservoir.state) %>%
  tab_header(title = md(
    "Table 6-82: Surface and Downstream CH4 Emissions from Reservoirs in Flooded Land Remaining Flooded Land in 2022 (kt CH4)")) %>%
  tab_footnote(footnote = "+ indicates values < 0.5 kt") %>%
    cols_label(
    name = "state", 
...
gt() %>%
  tab_header(title = md(
    "Table 6-83: IPCC (2019) Default CH_4 Emission Factors for Surface Emission from Reservoirs in Flooded Land Remaining Flooded Land")) %>%
  tab_footnote(footnote = "Note: downstream CH4 emissions are calculated as 9 percent of surface emissions. Downstream emissions are not calculated for CO2.") %>%
     cols_label(
       climate = "Climate",
