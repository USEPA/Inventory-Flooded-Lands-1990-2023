...
```{r}
flrfl.ch4.2022.reservoir.state.gt <- 
  fl.e$`2022` %>%  
...
# Create GT table of data
gt(flrfl.ch4.2022.reservoir.state.gt, rowname_col = "Source") %>%
  ...
  tab_style(style = cell_fill(color = "lightgray"),
            locations = list(
              cells_column_labels(), 
              cells_body(
                rows = seq(2, nrow(flrfl.ch4.2022.reservoir.state.gt), by = 2)))) %>%
  tab_style(style = cell_text(weight = "bold", color = "black"), 
...
fl.cl.area.state.type.gt <-fl.cl.area.state.type %>%
  filter(type == "reservoir",
         lu == "Flooded Land Remaining Flooded Land") %>%
...
# table
gt(flrfl.ch4.2022.ocwb.state.gt %>% 
     ungroup %>% 
     rename(State = name), rowname_col = "State") %>%
  tab_header(title = md(
    "Table 6-89: CH<sub>4</sub> Emissions from Other Constructed Waterbodies in Flooded Land Remaining Flooded Land in 2022 (kt CH<sub>4</sub>)")) %>%
  ...
  tab_style(style = cell_text(weight = "bold", color = "black"), 
    ...
    tab_style(style = cell_fill(color = "lightgray"),
            locations = list(
              ...
              cells_grand_summary(), cells_stub_grand_summary(),
              cells_stub(
                rows = seq(2, nrow(flrfl.ch4.2022.ocwb.state.gt), by = 2)),
              cells_body(
                rows = seq(2, nrow(flrfl.ch4.2022.ocwb.state.gt), by = 2)))) %>%
  tab_style(style = cell_text(align = "center"),
...
**Figure 6-13: 2022 CH~4~ Emissions from A) Ditches and Canals and B) Freshwater Ponds in Flooded Land Remaining Flooded Land (kt CH~4~)**
```{r, results="hide"}
# 1.  CALCULATE TOTAL OCWB CH4 EMISSIONS BY SUBTYPE AND STATE FOR 2022------------------
flrfl.ch4.2022.ocwb.state.gt # all emissions in kt ch4 year, see above
...
color.ditch <- data.frame(color.ditch = pal.blues(scales::rescale(flrfl.ch4.2022.ocwb.state.gt$ktonnes_ch4_canals_and_ditches)))
color.pond <- data.frame(color.pond = pal.blues(scales::rescale(flrfl.ch4.2022.ocwb.state.gt$ktonnes_ch4_freshwater_pond)))
...
flrfl.ch4.2022.ocwb.state.gt <- bind_cols(flrfl.ch4.2022.ocwb.state.gt, color.ditch, color.pond)

# add emissions and color data to object
us.plot.flrfl.ch4.2022.ocwb.state.gt <- merge(us, flrfl.ch4.2022.ocwb.state.gt)

# preview colors
us.plot.flrfl.ch4.2022.ocwb.state.gt


# 3. BREAK GEOGRAPHIES--------------
# CONUS
conus <- us.plot.flrfl.ch4.2022.ocwb.state.gt %>% # state + CH4
  filter(!name %in% c("Puerto Rico","Alaska","Hawaii"))%>%
  st_transform(5070)

# Alaska
alaska <- us.plot.flrfl.ch4.2022.ocwb.state.gt %>%
  filter(name == "Alaska")%>%
  st_transform(3467)

# Hawaii
hawaii <- us.plot.flrfl.ch4.2022.ocwb.state.gt %>%
  filter(name == "Hawaii")%>%
  st_transform(crs = 32604)

# Puerto Rico
pr <- us.plot.flrfl.ch4.2022.ocwb.state.gt %>%
  filter(name == "Puerto Rico") %>%
  st_transform(crs = 32619)
...
val.legend <-  seq(0, round(max(us.plot.flrfl.ch4.2022.ocwb.state.gt$ktonnes_ch4_canals_and_ditches), -1), by = 1)
pal.legend <- scales::rescale(val.legend)
...
# Load and render figure in markdown
knitr::include_graphics(c("output/figures/FLRFL.ocwb.pond.2022.Tch4.svg", "output/figures/FLRFL.ocwb.ditch.2022.Tch4.svg"))
...
<!--This table appears only in text.-->
```{r}
flrfl.ocwb.area.national.gt <- fl.cl.area.state.type %>%
  filter(type == "other constructed waterbodies",
         lu == "Flooded Land Remaining Flooded Land") %>%
  select(subtype, matches(c("1990|2005|2018|2019|2020|2021|2022"))) %>%
  group_by(subtype) %>%
...
**Figure 6-16: 2022 Surface Area of A) Ditches and Canals and B) Freshwater Ponds in Flooded Land Remaining Flooded Land (ha)**
```{r, results="hide"}
...
# Load figure and render in markdown
knitr::include_graphics(c("output/figures/FLRFL.ocwb.pond.2022.area.svg", "output/figures/FLRFL.ocwb.ditch.2022.area.svg"))
```
...
### QA/QC and Verification
The National Hydrography Data (NHD) is managed by the USGS in collaboration many other federal, state, and local entities. Extensive QA/QC procedures are incorporated into the curation of the NHD. The National Inventory of Dams (NID) is maintained by the U.S. Army Corps of Engineers (USACE) in collaboration with the Federal Emergency Management Agency (FEMA) and state regulatory offices. USACE resolves duplicative and conflicting data from 68 data sources, which helps obtain the more complete, accurate, and updated NID.^[See https://www.epa.gov/national-aquatic-resource-surveys/national-lakes-assessment-2017-quality-assurance-project-plan.] The Navigable Waterways (NW) dataset is part of the U.S. Department of Transportation (USDOT)/Bureau of Transportation Statistics (BTS) National Transportation Atlas Database (NTAD). The NW is a comprehensive network database of the nation's navigable waterways updated on a continuing basis. U.S. Fish and Wildlife Service is the principal agency in charge of wetland mapping including the National Wetlands Inventory (NWI). Quality and consistency of the Wetlands Layer is supported by federal wetlands mapping and classification standards, which were developed under the oversight of the Federal Geographic Data Committee (FGDC) with input by the FGDC Wetlands Subcommittee. This dataset is part of the FGDC Water-Inland Theme, which is co-chaired by the FWS and the U.S. Geological Survey. The EPA's Safe Drinking Water Information System (SDWIS) tracks information on drinking water contamination levels as required by the 1974 Safe Drinking Water Act and its 1986 and 1996 amendments.
...
### <!--FIGURE::  Aggregated reservoir emissions plotted by state-->
**Figure 6-18: 2022 A) CH~4~ and B) CO2 Emissions from U.S. Reservoirs in Land Converted to Flooded Land**
```{r, results="hide"}
...
# Load figure and render in markdown
knitr::include_graphics(c("output/figures/LCFL.reservoir.2022.Tch4.svg", "output/figures/LCFL.reservoir.2022.co2.svg"))
...
```{r}
# This table appears only in text.
fl.cl.area.state.type.gt.2 <- fl.cl.area.state.type %>%
  filter(type == "reservoir",
         lu == "Land Converted to Flooded Land") %>%
  select(subtype, matches(c("1990|2005|2018|2019|2020|2021|2022"))) %>%
  group_by(subtype) %>% # reservoir is only subtype in type == reservoir, thus not necessary
...
lcfl.ocwb.ghg.state.gt.2 <- rbind(lcfl.ocwb.ghg.state.2022, lcfl.ocwb.ghg.totals.2022) %>% 
  ...
  tab_style(style = cell_text(weight = "bold", color = "black"),
    ...
    tab_style(style = cell_fill(color = "lightgray"),
            locations = list(
              ...
              cells_grand_summary(), cells_stub_grand_summary(),
              cells_stub(
                rows = seq(2, nrow(lcfl.ocwb.ghg.state.gt.2), by = 2)),
              cells_body(
                rows = seq(2, nrow(lcfl.ocwb.ghg.state.gt.2), by = 2)))) %>%
  tab_style(style = cell_text(align = "right"),
            location = list(cells_body(), cells_column_labels(),
...
**Figure 6-21: 2022 A) CH~4~ and B) CO~2~ Emissions from Other Constructed Waterbodies (Freshwater Ponds) in Land Converted to Flooded Land (MT CO~2~ Eq.)**
```{r, results="hide"}
...
flooded_lands_script <- readLines("flooded_lands.Rmd")
...
writeLines(flooded_lands_cleaned, "flooded_lands_cleaned.Rmd")
