...
st_layers("inputData/FloodedLands2024.gpkg") # Flooded_Lands_2024
# read first row
# sf::st_read("~/shared/jbeaulie/inputData/Flooded_Lands_090424.gpkg",
#             query="select * from \"Flooded_Lands_2024\" limit 1") %>%
#   print(n=Inf)
# sf::st_read("inputData/FloodedLands2024.gpkg",
#             query="select * from \"FloodedLands_LAGOS_202\" limit 1") %>% # only first line
#   print(n=Inf)
...
fl.sf <- sf::st_read("inputData/FloodedLands2024.gpkg",
                   layer = "FloodedLands_LAGOS_2024") %>%
  janitor::clean_names() %>%
...
## <!--Estimate 2021 - 2023 Chlorophyll--!>
```{r}
# LAGOS trophic status data run from 1990:2020. 
# Assume 2021-2023 values are equal to 2020 values.
fl.sf <- fl.sf %>%
  mutate(chl_predicted_2021 = chl_predicted_2020,
         chl_predicted_2022 = chl_predicted_2020,
         chl_predicted_2023 = chl_predicted_2020)
...
fl.sf %>% st_drop_geometry() %>% 
  ...
  distinct(comid) %>%
    left_join(fl.sf) %>%
 summarise(n.nhd = n(), area.nhd = sum(area_ha_new)) #5,192 polygons, 1,784,966 ha
...
fl.sf %>% st_drop_geometry() %>% 
  ...
  distinct(nid_id) %>%
  left_join(fl.sf) %>%
 summarise(n.nwi = n(), area.nwi = sum(area_ha_new)) #19,515 polygons; 4,800,776 hectares
...
fl.sf <- fl.sf %>% tibble::rowid_to_column("unique_id")
...
format(object.size(fl.df), units = "GB") # 2.4GB
...
fl <- map(.x = rep("fl.df", 34), .f = get) %>% # `get` will find all instances of X, map dumps into list
  set_names(1990:2023) %>%
  # Each list element contains the full chl time series (e.g., 1984:2020).
  # restrict to only the year needed for specific list element based on list element name
  imap(~.x %>% 
         select(names(.x)[!(grepl("chl", names(.x)) & !grepl(.y, names(.x)))])) %>% # retain year-specific chl
  # strip "_year" from "chl_predicted_year" name
  map(~.x %>%
        rename_with(.fn = ~ sub('_\\d+$', '', .x), # replace terminal _ and subsequent characters with nothing
              .cols = starts_with("chl"))) # apply to all columns containing "chl" (1 column per list element)

format(object.size(fl), units = "auto") # 19.2 GB as dataframe!
...
# Modify EF to account for chlorophyll
fl.e <- map(fl.e, ~ .x %>%
              # mutate(ch4.tonnes.ha.y = case_when(subtype == "reservoir" & chl_predicted < 2.6 ~ ch4.tonnes.ha.y * 0.7,
              #                                    subtype == "reservoir" & chl_predicted > 2.6 & chl_predicted <= 20 ~ ch4.tonnes.ha.y * 3,
              #                                    subtype == "reservoir" & chl_predicted > 20 & chl_predicted <= 56 ~ ch4.tonnes.ha.y * 10,
              #                                    subtype == "reservoir" & chl_predicted > 56 ~ ch4.tonnes.ha.y * 25,
              #                                    TRUE ~ ch4.tonnes.ha.y)) %>%
              select(-contains("chl"))) # removing so code below doesn't break
...
## <!--Temporary code to evaluate trophic status adjustment impact-->
```{r ts_effect, eval=FALSE}

ch4.2023 <- fl.e$`2023` %>%
  group_by(lu, subtype) %>%
  summarize(tot_ch4 = sum(ch4.total.tonnes.y)/1000)

write_csv(ch4.2023, "output/2023_ch4_without_ts_adjustment.csv")
