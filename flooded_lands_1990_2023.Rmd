...
# presented as a single layer in FloodedLands2024.gpkg and shared
...
sf::st_read("inputData/FloodedLands2024.gpkg",
            # only first line
            query="select * from \"FloodedLands_LAGOS_2024\" limit 1") %>% 
  print(n=Inf)
...
```{r error_appendix, results="hide", eval=FALSE}
# 1990 first---------------
...
# ggplot(lcfl.ch4.co2.reservoir.national.change.perc %>%
#          pivot_longer(cols = everything()),
#        aes(name, value)) +
#   geom_point()
```
...
Overall, the recalculations resulted in minor changes in methane and carbon dioxide emissions from reservoirs in the early portion of the time series (mean change 1990 - 1993 = `r lcfl.ch4.co2.reservoir.national.change.perc %>% select(as.character(1990:1993)) %>% rowwise %>% summarize(p = mean(c_across(everything()))) %>% pull %>% round(1)`%) but differences between this *Inventory* and the 1990 through 2022 *Inventory* increase thereafter, with year 2022 emissions in the current *Inventory* exceeding those in the 1990 through 2022 *Inventory* by `r lcfl.ch4.co2.reservoir.national.change.perc %>% select(as.character(2022)) %>% pull %>% round(1)`%.
...
# LCFL - ocwb CO2 CH4 from current inventory: 1990-2022
