...
gt(ch4.co2.uncertainty.converted.res.gt, 
   rowname_col = "subtype") %>%
  tab_header(title = md(
    "Table 6-109: Approach 2 Quantitative Uncertainty Estimates for CH~4~ and CO~2~ Emissions from Reservoirs in Land Converted to Flooded Land")) %>% 
  tab_stub_indent(
    rows = everything(),
    indent = 3) %>%
  cols_label(Gas = "",
...
ch4.co2.uncertainty.converted.gt <- ch4.co2.uncertainty.mean.table %>%
  filter(lu == "Land Converted to Flooded Land", 
         subtype == "freshwater pond", year == 2023) %>%
