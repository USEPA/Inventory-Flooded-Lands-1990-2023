...
```{r, flooded_lands_data_sources, message=FALSE, warning=FALSE, results="hide", cache=TRUE}
...
fl.sf <- sf::st_read("inputData/Flooded_Lands_090424.gpkg",
  ...
  rename(area_meters = intersect_area_meters,
         stusps = state_abr,
         year = dam_year_completed,
         climate = fldd_zn) 
toc() # 167 seconds on laptop

# subsample for fast knitting
fl.sf <- fl.sf %>% slice_sample(prop = 0.1)
...
fl.sf %>% st_drop_geometry() %>% # expedite calcs
  summarize(dam.n = sum(!is.na(year))) #18,708
...
gt(flrfl.ch4.2023.reservoir.state.gt, rowname_col = "Source") %>%
  ...
  tab_style(style = cell_fill(color = "lightgray"),
            locations = list(
              cells_column_labels(), 
              cells_body(
                rows = seq(2, nrow(flrfl.ch4.2023.reservoir.state), by = 2)))) %>%
  tab_style(style = cell_text(weight = "bold", color = "black"), 
...
The net effect of these recalculations was an average annual increase in CH4 emission estimates from reservoirs of 1.23 MMT CO2 Eq., or 4 percent, over the time series from 1990 to 2021 compared to the previous Inventory.
...
Methane emissions from freshwater ponds in flooded land remaining flooded land increased by approximately `r ((flrfl.ch4.national.subtype %>% filter(subtype == "freshwater pond", flux.type == "total") %>% select("2022") %>% pull() -  flrfl.ch4.national.subtype %>% filter(subtype == "freshwater pond", flux.type == "total") %>% select("1990") %>% pull()) /  flrfl.ch4.national.subtype %>% filter(subtype == "freshwater pond", flux.type == "total") %>% select("2022") %>% pull()) * 100` percent from 1990 to 2022. Methane emissions from canals and ditches have remained constant throughout the time series because age data are not available for canals and ditches, thus they are assumed to be greater than 20-years old in 1990 and are included in flooded land remaining flooded land throughout the time series. Overall, CH4 emissions from other constructed waterbodies have remained fairly constant since 1990 (Table 6-87 and Table 6-88).
...
gt(flrfl.ch4.2023.ocwb.state.gt %>% 
  ...
  tab_header(title = md(
    "Table 6-8??: CH<sub>4</sub> Emissions from Other Constructed Waterbodies in Flooded Land Remaining Flooded Land in 2023 (kt CH<sub>4</sub>)")) %>%
...
flrfl.ch4.2023.ocwb.state # all emissions in kt ch4 year, see above
...
color.ditch <- data.frame(color.ditch = pal.blues(scales::rescale(flrfl.ch4.2023.ocwb.state$ktonnes_ch4_canals_and_ditches)))
color.pond <- data.frame(color.pond = pal.blues(scales::rescale(flrfl.ch4.2023.ocwb.state$ktonnes_ch4_freshwater_pond)))
...
flrfl.ch4.2023.ocwb.state <- bind_cols(flrfl.ch4.2023.ocwb.state, color.ditch, color.pond)

# add emissions and color data to object
us.plot.flrfl.ch4.2023.ocwb.state <- merge(us, flrfl.ch4.2023.ocwb.state)

# preview colors
us.plot.flrfl.ch4.2023.ocwb.state


# 3. BREAK GEOGRAPHIES--------------
# CONUS
conus <- us.plot.flrfl.ch4.2023.ocwb.state %>% # state + CH4
  filter(!name %in% c("Puerto Rico","Alaska","Hawaii"))%>%
  st_transform(5070)

# Alaska
alaska <- us.plot.flrfl.ch4.2023.ocwb.state %>%
  filter(name == "Alaska")%>%
  st_transform(3467)

# Hawaii
hawaii <- us.plot.flrfl.ch4.2023.ocwb.state %>%
  filter(name == "Hawaii")%>%
  st_transform(crs = 32604)

# Puerto Rico
pr <- us.plot.flrfl.ch4.2023.ocwb.state %>%
  filter(name == "Puerto Rico") %>%
  st_transform(crs = 32619)
...
mf_export(x = conus,
          filename = "output/figures/FLRFL.ocwb.ditch.2023.Tch4.svg",
...
val.legend <-  seq(0, round(max(us.plot.flrfl.ch4.2023.ocwb.state$ktonnes_ch4_canals_and_ditches), -1), by = 1)
pal.legend <- scales::rescale(val.legend)
...
# 6. MAKE PONDS MAP-------------------------
mf_export(x = conus,
          filename = "output/figures/FLRFL.ocwb.pond.2023.Tch4.svg",
          width = 7, expandBB = c(.3,.3,0.1,.1)) # (bottom, left, top, right)
#mf_shadow(cs, add = TRUE,col = "grey50")
...
# Load and render figure in markdown
knitr::include_graphics(c("output/figures/FLRFL.ocwb.pond.2023.Tch4.svg", "output/figures/FLRFL.ocwb.ditch.2023.Tch4.svg"))
...
mf_export(x = conus,
          filename = "output/figures/FLRFL.ocwb.ditch.2023.area.svg",
          width = 7, expandBB = c(.3,.3,0.1,.1)) # (bottom, left, top, right)
#mf_shadow(cs, add = TRUE,col = "grey50")
...
# .svg vector format works, but DMAP won't render a view.  Must download to local computer, then view.
mf_export(x = conus,
          filename = "output/figures/FLRFL.ocwb.pond.2023.area.svg",
          width = 7, expandBB = c(.3,.3,0.1,.1)) # (bottom, left, top, right)
#mf_shadow(cs, add = TRUE,col = "grey50")
...
```{r}
# Load figure and render in markdown
knitr::include_graphics(c("output/figures/FLRFL.ocwb.pond.2023.area.svg", "output/figures/FLRFL.ocwb.ditch.2023.area.svg"))
```
...
### Uncertainty
Uncertainty in estimates of CH4 emissions from other constructed waterbodies (ponds, canals/ditches) in flooded land remaining flooded land (Table 6-96) are estimated using IPCC Approach 2 and include uncertainty in the default emission factors and the flooded land area inventory. Uncertainty in default emission factors is provided in the *2019 Refinement to the 2006 IPCC Guidelines* (IPCC 2019). Uncertainties in the spatial data include 1) uncertainty in area estimates from the NHD, NWI, and NW, and 2) uncertainty in the location of dams in the NID. Overall uncertainties in these spatial datasets are unknown, but uncertainty for remote sensing products is assumed to be ± 10 to 15 percent based on IPCC guidance (IPCC 2003). An uncertainty range of ± 15 percent for the flooded land area estimates is assumed and is based on expert judgment.
...
mf_export(x = conus,
          filename = "output/figures/LCFL.reservoir.2023.Tch4.svg",
...
mf_export(x = conus,
          filename = "output/figures/LCFL.reservoir.2023.co2.svg",
...
```{r}
# Load figure and render in markdown
knitr::include_graphics(c("output/figures/LCFL.reservoir.2023.co2.svg", "output/figures/LCFL.reservoir.2023.Tch4.svg"))
...
gt(ef.gt.3) %>%
  ...
  cols_label(
    climate = "Climate",
    ch4.tonnes.ha.y = "Mt {{CH_4 ha^-1 y^-1}}",
    co2.tonnes.ha.y = "Mt {{CO_2 ha^-1 y^-1}}") %>%
  fmt_number(decimals = 4) %>%
  tab_options(
    heading.border.bottom.style = "solid", 
    heading.border.bottom.color = "black",
...
```{r}
# Load and render figure in markdown
knitr::include_graphics("output/figures/newDams.tiff")
...
##CURRENTLY NO WATERBODIES IN THIS GROUP.  MAKE SOMETHING UP TO POPULATE TABLES-->
...
These figures need to be coerced into a 2-column by 1-row image. 
...
mf_export(x = conus,
          filename = "output/figures/LCFL.ocwb.2023.Tch4.svg",
          width = 7, expandBB = c(.3,.3,0.1,.1))
#mf_shadow(cs, add = TRUE,col = "grey50")
...
mf_export(x = conus,
          filename = "output/figures/LCFL.ocwb.2023.co2.svg",
          width = 7, expandBB = c(.3,.3,0.1,.1)) # original width = 1200, shrinking to accommodate 2 panels
#mf_shadow(cs, add = TRUE,col = "grey50")
...
```{r}
# Load figure and render in markdown
knitr::include_graphics(c("output/figures/LCFL.ocwb.2022.Tch4.svg", "output/figures/LCFL.ocwb.2022.co2.svg"))
...
gt(ef.gt.4) %>%
  ...
  cols_label(
    ...
    ch4.tonnes.ha.y = "Surface emission factor (Mt {{CH_4 ha^-1 y^-1}})",
    co2.tonnes.ha.y = "Surface emission factor (Mt {{CO_2 ha^-1 y^-1}})") %>%
  fmt_number(decimals = 4) %>%
  tab_options(
    heading.border.bottom.style = "solid", 
    heading.border.bottom.color = "black",
...
mf_export(x = conus,
          filename = "output/figures/LCFL.ocwb.pond.2023.area.svg",
          width = 7, expandBB = c(.3,.3,0.1,.1)) # (bottom, left, top, right)
#mf_shadow(cs, add = TRUE,col = "grey50")
...
```{r}
# Load figure and render in markdown
knitr::include_graphics("output/figures/LCFL.ocwb.pond.2023.area.svg")
