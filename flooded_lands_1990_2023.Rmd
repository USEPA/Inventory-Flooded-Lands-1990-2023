...
if (file.exists("output/fl.error.sim.i.rds") &
    file.exists("output/fl.error.sim.subtype.rds") &
    file.exists("output/fl.error.sim.type.rds")) {
  # If it exists, load it
  fl.error.sim.i <- readRDS("output/fl.error.sim.i.rds")
  fl.error.sim.subtype <- readRDS("output/fl.error.sim.subtype.rds")
  fl.error.sim.type <- readRDS("output/fl.error.sim.type.rds")
  message("Uncertainty objects loaded from file.")
  # If it doesn't exist, then run the simulations
} else {
  for (i in 1:10000) { #
    message(i) # progress indicator printed duing render
  ...
  saveRDS(fl.error.sim.i, file = "output/fl.error.sim.i.rds")
  saveRDS(fl.error.sim.subtype, file = "output/fl.error.sim.subtype.rds")
  saveRDS(fl.error.sim.type, file = "output/fl.error.sim.type.rds") 
}
...
ch4.co2.uncertainty.mean.table <- merge(ch4.co2.uncertainty.table, # min and max values from simulation by subtype
                                        ch4.co2.mean.table) %>% # mean values from calculations, not simulations, by subtype
...
ch4.co2.uncertainty.mean.table.total <- merge(ch4.co2.uncertainty.table.total, # min and max values from simulation by type (aggregates across canal./pond)
                                              ch4.co2.mean.table.total) %>% # mean from calculations
  mutate(upr.perc.ch4.surface = bound.perc(ch4.surface.mmt.co2.eq.y_max,
                                           ch4.surface.mmt.co2.eq.y),
         lwr.perc.ch4.surface = bound.perc(ch4.surface.mmt.co2.eq.y_min,
                                           ch4.surface.mmt.co2.eq.y),
...
#-##-##-##-##-##-##-##-##-##-##-##-##-##-#
# FOLLOWING CODE WAS USED TO GENERATE UNCERTAINTY TABLES IN EARLY INVENTORY
# CYCLES. RETAINING CODE TO HELP DEVELOP/TROUBLESHOOT GT UNCERTAINTY TABLES
...
ch4.co2.uncertainty.gt <- ch4.co2.uncertainty.mean.table %>%
      filter(lu == "Flooded Land Remaining Flooded Land",
             subtype == "reservoir", year == final_year) %>%
  select(contains("ch4")) %>%
   pivot_longer(cols = contains("ch4"), names_to = "variable", values_to = "value") %>%
  mutate(subtype = case_when(str_detect(variable, "surface") ~ "Surface",
                             str_detect(variable, "dn") ~ "Downstream",
  ...
  mutate(variable = case_when(
         ...
         variable == "ch4.total.mmt.co2.eq.y" ~ "ch4_est", 
         variable == "ch4.total.mmt.co2.eq.y_min" ~ "ch4_min", 
         variable == "ch4.total.mmt.co2.eq.y_max" ~ "ch4_max", 
         variable == "lwr.perc.ch4.total" ~ "ch4_low_per", 
         variable == "upr.perc.ch4.total" ~ "ch4_up_per")) %>%
  pivot_wider(names_from = variable, values_from = value) %>%
  relocate(subtype, ch4_est, ch4_min, ch4_max, ch4_low_per, ch4_up_per)

# Base year for Excel only
ch4.co2.uncertainty.gt.base <- ch4.co2.uncertainty.mean.table %>%
      filter(lu == "Flooded Land Remaining Flooded Land",
...
in Flooded Land Remaining Flooded Land")) %>% 
  ...
  tab_options(
    ...
    grand_summary_row.border.color = "black",
    grand_summary_row.border.width = 3) %>%
  # Totals must come from calculations above, not from gt calculations
  # grand_summary_rows(
  #   columns = where(is.numeric), 
  #   fmt = ~ fmt_number(.), 
  #   fns = list(Total ~ sum(., na.rm = TRUE))) %>%
...
ch4.co2.uncertainty.other.gt <- bind_rows(
  # Current year for Markdown and Excel
  ch4.co2.uncertainty.mean.table %>%
    filter(lu == "Flooded Land Remaining Flooded Land",
           subtype %in% c("canals and ditches", "freshwater pond"),
  ...
  select(subtype,
         ch4_est = ch4.total.mmt.co2.eq.y,
         ch4_min = ch4.total.mmt.co2.eq.y_min,
         ch4_max = ch4.total.mmt.co2.eq.y_max, 
         ch4_low_per = lwr.perc.ch4.total,  
         ch4_up_per = upr.perc.ch4.total)


# Base year for Excel only
ch4.co2.uncertainty.other.gt.base <- bind_rows(
  # Current year for Markdown and Excel
  ch4.co2.uncertainty.mean.table %>%
    filter(lu == "Flooded Land Remaining Flooded Land",
           subtype %in% c("canals and ditches", "freshwater pond"),
...
lcfl.ghg.national.subtype.gt.4 <- lcfl.ghg.national.subtype %>% 
  filter(subtype == "reservoir", gas == "co2", units == "mmtc") %>%
  select(Source = Activity, "1990", "2005", as.character((final_year-4):final_year)) 
...
Minnesota was the largest source of CH~4~ and CO~2~ emissions from reservoirs in land converted to flooded land, constituting `r lcfl.ghg.2023.reservoir.mn.ratio %>% round(.,1)` percent of total emissions from this category (Figure 6-18 and Table 6-105). These emissions are attributed to `r fl.e %>% pluck(as.character(final_year)) %>% filter(lu == "Land Converted to Flooded Land", type == "reservoir", name == "Minnesota") %>% summarize(n = n()) %>% pull() %>% xfun::numbers_to_words(.)` reservoirs created after 2001 which impounded `r fl.e %>% pluck(as.character(final_year)) %>% filter(lu == "Land Converted to Flooded Land", type == "reservoir", name == "Minnesota") %>% summarize(area = sum(area_ha_new)) %>% pull() %>% format(big.mark = ",", digits = 1, scientific = FALSE)` ha of water, `r (fl.e %>% pluck(as.character(final_year)) %>% filter(comid == 2360642) %>% select(area_ha_original) %>% pull() / fl.e %>% pluck(as.character(final_year)) %>% filter(lu == "Land Converted to Flooded Land", type == "reservoir", name == "Minnesota") %>% summarize(area = sum(area_ha_new)) %>% pull()) %>% "*"(100) %>% round(0)` percent of which is located in Mille Lacs lake.

North Dakota is the second largest source of CO~2~ and CH~4~ from reservoirs in land converted to flooded land. Over `r (fl.e %>% pluck(as.character(final_year)) %>% filter(lu == "Land Converted to Flooded Land", type == "reservoir", name == "North Dakota") %>% slice_max(area_ha_new) %>% select(area_ha_new) %>% pull() / fl.e %>% pluck(as.character(final_year)) %>% filter(lu == "Land Converted to Flooded Land", type == "reservoir", name == "North Dakota")  %>% summarize(area=sum(area_ha_new)) %>% pull()) %>% "*"(100) %>% round(1)` percent of land converted to flooded land reservoir surface area in North Dakota is attributed to Devils Lake. Both Mille Lacs and Devils Lakes are natural waterbodies provisioned with dams for water level management. The dams have not substantially increased the surface area of these reservoirs, but serve to reduce flooding and/or maintain minimum water levels for recreation, thereby affecting water residence time.
...
fl.cl.area.state.type.gt.2 <- 
  ...
  filter(type == "reservoir",
         lu == "Land Converted to Flooded Land") %>%
  select(subtype, matches(paste0(c(1990, 2005, (final_year-4):final_year), collapse = "|"))) %>%
  group_by(subtype) %>% # reservoir is only subtype in type == reservoir, thus not necessary
  summarize(across(contains("area"), ~sum(./1000))) %>% # thousands of ha
  arrange(desc(subtype)) %>%
  # Rename columns for the table by extracting the 4-digit year
  rename_with(~str_extract(., "\\d+"), starts_with("area"))
...
ch4.co2.uncertainty.converted.res.gt <- ch4.co2.uncertainty.mean.table %>%
  ...
  mutate(subtype = case_when(str_detect(variable, "surface") ~ "Surface",
                             str_detect(variable, "dn") ~ "Downstream",
                             str_detect(variable, "total") ~ "Total",
                             str_detect(variable, "co2") ~ "Surface", # only surface emissions for CO2
                             TRUE ~ "Fly you fools!")) %>% # error code
  mutate(Gas = case_when(str_detect(variable, "ch4") ~ "CH4",
                         subtype == "Total" & str_detect(variable, "ghg") ~ "", # this is CH4 + CO2 
                         str_detect(variable, "co2") ~ "CO2",
                         TRUE ~ "Fly you fools!")) %>% # error code
  mutate(variable = case_when(
    # ch4 surface
    ...
    # ghg total
    variable == "ghg.total.mmt.co2.eq.y" ~ "est",
    variable == "ghg.total.mmt.co2.eq.y_min" ~ "min",
    variable == "ghg.total.mmt.co2.eq.y_max" ~ "max",
    variable == "lwr.perc.ghg.total" ~ "low_per",
    variable == "upr.perc.ghg.total" ~ "up_per",
    # surface + downstream CH4, not needed
    str_detect(variable, "ch4.total") ~ "ch4_total", 
    # error code
    TRUE ~ "Fly you Fools!")) %>% 
  filter(!(variable == "ch4_total")) %>%
  pivot_wider(names_from = variable, values_from = value) %>%
  arrange(match(subtype, c("Surface", "Downstream", "Total"))) %>%
  relocate(subtype, Gas, est, min, max, low_per, up_per)
...
# Current year for Markdown and Excel
ch4.co2.uncertainty.converted.gt <- bind_rows(
  # mean part of table
  ch4.co2.uncertainty.mean.table %>%
    filter(lu == "Land Converted to Flooded Land",
           subtype %in% c("freshwater pond", "canals and ditches"),
           year == final_year) %>%
    select(subtype, contains("total"), starts_with("co2"), contains("perc")) %>%
    select(-contains("ghg"), -contains("dn")) %>%
    mutate(across(!contains("perc") & -subtype, ~ .x * 1000)) %>% # convert to ktonnes!!!!
    pivot_longer(cols = !subtype, names_to = "variable", 
                 values_to = "value") %>%
    mutate(Gas = case_when(str_detect(variable, "ch4") ~ "CH4",
                           subtype == "Total" & str_detect(variable, "ghg") ~ "", # this is CH4 + CO2 
                           str_detect(variable, c("co2.mmt|perc.co2")) ~ "CO2",
                           TRUE ~ "Fly you fools!")) %>% # error code
    mutate(variable = case_when(
      variable == "ch4.total.mmt.co2.eq.y" ~ "est", 
      variable == "ch4.total.mmt.co2.eq.y_min" ~ "min", 
      variable == "ch4.total.mmt.co2.eq.y_max" ~ "max", 
      variable == "lwr.perc.ch4.total" ~ "low_per", 
      variable == "upr.perc.ch4.total" ~ "up_per", 
      variable == "co2.mmt.co2.eq.y" ~ "est",
      variable == "co2.mmt.co2.eq.y_min" ~ "min", 
      variable == "co2.mmt.co2.eq.y_max" ~ "max", 
      variable == "lwr.perc.co2" ~ "low_per", 
      variable == "upr.perc.co2" ~ "up_per",
      variable == "upr.perc.ch4.surface" ~ "not_needed", # redundant with total ch4
      variable == "lwr.perc.ch4.surface" ~ "not_needed", # redundant with total ch4
      TRUE ~ "Fly you fools!")) %>%
    filter(variable != "not_needed") %>%
    pivot_wider(names_from = variable, values_from = value) %>%
  relocate(subtype, Gas, est, min, max, low_per, up_per),
...
filter(lu == "Land Converted to Flooded Land",
       ...
       year == final_year) %>%
      select(contains("ghg")) %>%
      mutate(across(!contains("perc"), ~ .x * 1000), # convert to ktonnes!!!!
             subtype = "Total") %>%
      pivot_longer(cols = !subtype, names_to = "variable", 
                 values_to = "value") %>%
  mutate(variable = case_when(
...
# Base year for Excel only
ch4.co2.uncertainty.converted.gt.base <- bind_rows(
  # mean part of table
  ch4.co2.uncertainty.mean.table %>%
    filter(lu == "Land Converted to Flooded Land",
           subtype %in% c("freshwater pond", "canals and ditches"),
...
gt(ch4.co2.uncertainty.converted.gt, 
  ...
  tab_spanner(label = "Uncertainty Range Relative to Emission Estimate", 
              columns = 4:7, level = 2, id = "range") %>%
  tab_spanner(label = "(kt CO2 Eq.)", columns = 3:3, level = 1) %>%
  tab_spanner(label = "(kt CO2 Eq.) ", columns = 4:5, level = 1) %>%
  tab_spanner(label = "(%)", columns = 6:7, level = 1) %>%
  opt_horizontal_padding(2) %>%
  tab_options(
    heading.border.bottom.style = "solid", 
