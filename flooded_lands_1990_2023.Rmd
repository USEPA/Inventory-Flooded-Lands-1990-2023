...
### FIGURE: Climate zone and Flooded lands map{display: none}
...
### FIGURE::  Aggregated reservoir emissions plotted by state{.hidden}
{.visible}
**Figure 6-11: Total CH4 Emissions (Downstream + Surface) from Reservoirs in Flooded Land Remaining Flooded Land in 2022 (kt CH4)**
```{r, results="hide"}
...
val.legend <-  seq(0, max(us.plot.flrfl.total.ch4.2022.reservoir$ch4.total.ktonnes.y), by = 20)
...
```
**Figure 6-11:  Total CH[4] Emissions (Downstream + Surface) from Reservoirs in Flooded Land Remaining Flooded Land in 2022 (kt CH[4])**
```{r}
...
### TABLE:: FLRFL-reservoirs CH4 emissions by mechanism (dn, surface) and state{.hidden}
...
# Create GT table of data
gt(flrfl.ch4.2022.reservoir.state, rowname_col = "Source") %>%
  tab_header(title = md( "Table 6-82: Surface and Downstream CH<sub>4</sub> Emissions from Reservoirs in Flooded Land Remaining Flooded Land in 2022 (kt CH<sub>4</sub>)")) %>%
...
#### TABLE: default emission factors{.hidden}
{.visible}
```{r}
ef %>%
...
##### Area estimates
U.S. reservoirs were identified from the NHDWaterbody layer in the National Hydrography Dataset Plus V2 (NHD),66 the National Inventory of Dams (NID),67 the National Wetlands Inventory (NWI),68 the Navigable Waterways (NW) network,69 and the EPA’s Safe Drinking Water Information System (SDWIS).70 The NHD only covers the conterminous U.S., whereas the NID, NW and NWI also include Alaska, Hawaii, and Puerto Rico.
...
Reservoirs were disaggregated by state (using boundaries from the 2016 U.S. Census Bureau71) and climate zone. Downstream and surface emissions for cross-state reservoirs were allocated to states based on the surface area that the reservoir occupied in each state. Only the U.S. portion of reservoirs that cross country borders were included in the inventory.
...
######TABLE:: National Flooded Land Area for Reservoirs{.hidden}
{.visible}
```{r}
...
######TABLE:: Flooded Land Remaining Flooded Land Area for Reservoirs by State: abbreviated time series{.hidden}
...
# This table contains only 1990, 2005, 2018-2022.
...
```
######TABLE:: Flooded Land Remaining Flooded Land Area for Reservoirs by State: all years{.hidden}
...
The EPA recently completed a survey of greenhouse gas emissions from 108 reservoirs in the conterminous United States.72 The data will be used to develop country-specific emission factors for U.S. reservoirs to be used in the 1990 through 2024 Inventory submission.
...
### TABLE:: National level CH4 flux table{.hidden}
...
### TABLE:: State level CH4 flux table{.hidden}
...
# table
gt(flrfl.ch4.2022.ocwb.state %>% 
     ungroup %>% 
     rename(State = name), rowname_col = "State") %>%
  tab_header(title = md(
    "Table 6-8??: CH<sub>4</sub> Emissions from Other Constructed Waterbodies in Flooded Land Remaining Flooded Land in 2022 (kt CH<sub>4</sub>)")) %>%
...
### FIGURE: Emissions by state{.hidden}
{.visible}
**Figure 6-13: 2022 CH4 Emissions from A) Ditches and Canals and B) Freshwater Ponds in Flooded Land Remaining Flooded Land (kt CH4)**
```{r, results="hide"}
...
```
##### Area Estimates
Other constructed waterbodies were identified from the NHDWaterbody layer in the National Hydrography Dataset Plus V2 (NHD), the National Inventory of Dams (NID), the National Wetlands Inventory (NWI), the Navigable Waterways (NW) network, and the EPA’s Safe Drinking Water Information System (SDWIS). The NHD only covers the conterminous United States, whereas the NID, NW and NWI also include Alaska, Hawaii, District of Columbia, and Puerto Rico. The following paragraphs present the criteria used to identify other constructed waterbodies in the NHD, NW, and NWI.
...
###### FIGURE: NWI features identified as canals/ditches{.hidden}
{.visible}
...
###### TABLE: Predictors used in Decision Tree to Identify Canal/Ditches{.hidden}
{.visible}
```{r}
tibble(Predictor = c("Shape Length : # of Shape Vertices",
...
###### TABLE: Validation Results for Ditch/Canal Classification Decision Tree{.hidden}
{.visible}
```{r}
# needs "Truth" header obove the 2nd and 3rd columns.
tribble(~Prediction, ~`Ditch/Canal`, ~`Not Ditch/Canal`,
        "Ditch/Canal", 49,          5,
...
###### FIGURE: Canals/Ditch Decision Tree Structure{.hidden}
{.visible}
```{r}
# need to add title
...
###### TABLE:: National Flooded Land Area for Other Constructed Waterbodies: abbreviated time series{.hidden}
This table appears only in text.
{.visible}
```{r}
...
###### TABLE:: National Flooded Land Area for Other Constructed Waterbodies: full time series{.hidden}
...
###### FIGURE:: Flooded Land Area for Other Constructed Waterbodies by State{.hidden}
{.visible}
```{r, results="hide"}
...
###### TABLE:: Flooded Land Area for Other Constructed Waterbodies-canals/ditches by State: abbreviated time series{.hidden}
...
# Create a GT table of the data
gt(flrfl.canal.area.by.state %>% rename(State = name) %>%
  rename_with(~str_extract(., "\\d+"), starts_with("area")), 
  rowname_col = "State") %>%
  tab_header(title = md(
...
###### TABLE:: Flooded Land Area for Other Constructed Waterbodies-canals and ditches by State: all years{.hidden}
...
###### TABLE:: Flooded Land Area for Other Constructed Waterbodies-freshwater ponds by State: abbreviated time series{.hidden}
...
###### TABLE:: Flooded Land Area for Other Constructed Waterbodies-freshwater ponds by State: all years{.hidden}
...
#### TABLE:: Uncertainty Estimates{.hidden}
{.visible}
```{r}
...
Need to add Note.
{.visible}
**Figure 6-17: U.S. Reservoirs (black polygons) in the Land Converted to Flooded Land Category in 2022**
```{r, results="hide"}
...
### TABLE::  GHG Emissions from Land Converted to Flooded Land—Reservoirs{.hidden}
...
lcfl.ghg.national.subtype %>% 
  ...
  gt() %>%
  tab_header(title = md(
    "Table 6-103: CO2 Emissions from Land Converted to Flooded Land—Reservoirs (MMT CO2)")) %>%
  fmt_number(decimals = 4) %>%
...
### TABLE:: LCFL-reservoirs CH4 and CO2 emissions by mechanism (dn, surface) and state{.hidden}
{.visible}
```{r}
...
# Create a GT table of the data
gt(lcfl.res.ghg.by.state %>% 
     ungroup %>% 
     select(State = name, contains("ktonnes")), 
   rowname_col = "State") %>%
  tab_header(title = md(
    "Table 6-105: Methane and **CO<sub>2<sub>** Emissions from Reservoirs in Land Converted to Flooded Land in 2022 (kt **CH<sub>4<sub>**; kt **CO<sub>2<sub>**)")) %>%
  cols_label(ch4.surface.ktonnes.y = md("**CH<sub>4<sub>** Surface"),
    ch4.dn.ktonnes.y	 = md("**CH<sub>4<sub>** Downstream"),
 	  ch4.total.ktonnes.y = md("**CH<sub>4<sub>** Total"), 
  ...
  tab_style(style = cell_text(align = "center"),
            location = list(cells_body(), cells_grand_summary(), 
                            cells_column_labels())) %>%
  tab_footnote(footnote = "+ Indicates values less than 0.5 kt.") %>%
  tab_footnote(footnote = "a **CO<sub>2<sub>**: Only surface **CO<sub>2<sub>** emissions are included in the Inventory.")
...
#### TABLE: Default Emission Factor{.hidden}
{.visible}
```{r}
ef %>%
  filter(subtype == "reservoir",
...
###### FIGURE: Selected Features from NWI that meet Flooded Lands Criteria{.hidden}
{.visible}
...
##### TABLE:: Flooded Land Area for LCFL Reservoirs by State: abbreviated time series{.hidden}
{.visible}
```{r}
...
##### TABLE:: Flooded Land Area for LCFL Reservoirs by State: full time series{.hidden}
...
#### NOTES:: Calculations to inform discussion{.hidden}
...
### Planned Improvements
The EPA recently completed a survey of greenhouse gas emissions from 108 reservoirs in the conterminous United States.91 The data will be used to develop country-specific emission factors for U.S. reservoirs to be used in the 1990 through 2024 Inventory submission.
...
### TABLE:: National level CH4 and CO2 flux table{.hidden}
...
lcfl.ghg.national.subtype %>%
  ...
  gt() %>%
  tab_header(title = md(
    "Table 6-110: CH4 Emissions from Other Constructed Waterbodies in Land Converted to Flooded Land (MMT CO2 Eq.)")) %>%
...
lcfl.ghg.national.subtype %>%
  ...
  gt() %>%
  tab_header(title = md(
    "Table 6-111: CH4 Emissions from Other Constructed Waterbodies in Land Converted to Flooded Land (kt CH4)")) %>%
  tab_footnote(footnote = "+ Indicates values less than 0.5 kt.") %>%
  fmt_number(decimals = 4) %>%
  opt_stylize(style = 5, color = "gray") 
...
lcfl.ghg.national.subtype %>%
  ...
  gt() %>%
  tab_header(title = md(
    "Table 6-112: CO2 Emissions from Other Constructed Waterbodies in Land Converted to Flooded Land (MMT CO2)")) %>%
...
lcfl.ghg.national.subtype %>%
  ...
  gt() %>%
  tab_header(title = md(
    "Table 6-113: CO2 Emissions from Other Constructed Waterbodies in Land Converted to Flooded Land (MMT C)")) %>%
  tab_footnote(footnote = "+ Indicates values less than 0.05 MMT C.") %>%
  fmt_number(decimals = 4) %>%
  opt_stylize(style = 6, color = "blue") 
```

### TABLE:: State level GHG flux table{.hidden}
{.visible}
```{r}
...
to Flooded Land in 2022 (MT **CO<sub>2<sub>** Eq.)")) %>%
  tab_stubhead(label = "State") %>%
  cols_label(
...
### FIGURE 6-21:: Emissions by state{.hidden}
These figures need to be coerced into a 2-column by 1-row image. 
{.visible}
**Figure 6-21: 2022 A) CH4 and B) CO2 Emissions from Other Constructed Waterbodies (Freshwater Ponds) in Land Converted to Flooded Land (MT CO2 Eq.)**
```{r, results="hide"}
...
##### TABLE:: National Flooded Land Area for Other Constructed Waterbodies{.hidden}
{.visible}
```{r}
fl.cl.area.state.type %>%
  filter(type == "other constructed waterbodies",
...
##### FIGURE:: Flooded Land Area for Other Constructed Waterbodies by State{.hidden}
{.visible}
**Figure 6-22: Surface Area of Other Constructed Waterbodies in Land Converted to Flooded Land (ha) in 2022**
```{r, results="hide"}
...
##### TABLE:: Flooded Land Area for Other Constructed Waterbodies by State: abbreviated time series{.hidden}
{.visible}
```{r}
...
##### TABLE:: Flooded Land Area for Other Constructed Waterbodies by State: full time series{.hidden}
